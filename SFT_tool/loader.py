...
class Data_conversion(IterableDataset):
    ...
    def list_flatten(self,lst,tokenizer,add_special_tokens):
        result = []
        for item in lst:
            if isinstance(item, list):
                result.extend(self.list_flatten(item,tokenizer,add_special_tokens))
            else:
                result.append(item) if add_special_tokens == False else result.append(f"{tokenizer.bos_token}{item}{tokenizer.eos_token}")
        return ["".join(result)]
    ...
    def data_conversion(self, tokenizer, input_idex, history_idx, label_field, max_seqlen, max_length_limit, add_special_token):
        for sample in self.data:
            ...
            parts = ["".join(parts)] if add_special_token == False else [f"{tokenizer.bos_token}{''.join(parts)}{tokenizer.eos_token}"]
            text_tensors = tokenizer(parts, padding=False, truncation=True, max_length=2048, return_tensors="pt",
                                     add_special_tokens=False)
            text_tensors_cat = text_tensors
            if history_idx is not None:
                for idx in history_idx:
                    history = self.list_flatten(sample[idx],tokenizer,add_special_token)
                    if text_tensors["input_ids"].shape[1] <= max_length_limit:
                        history_tensors = tokenizer(history, padding=False, truncation=True, max_length=2048, return_tensors="pt",
                                     add_special_tokens=False)["input_ids"]
                        cat_tensors = torch.cat([history_tensors[:,:max_length_limit-text_tensors["input_ids"].shape[1]],text_tensors["input_ids"]],dim=1)
                        text_tensors_cat = {"input_ids":cat_tensors.to(text_tensors.dtype),"attention_mask":torch.ones_like(cat_tensors).to(text_tensors.dtype)}

            if label_field == None:
                labels_tensors = text_tensors_cat
            else:
                label = []
                ...
                labels_tensors = tokenizer(label, padding=False, truncation=True, max_length=2048, return_tensors="pt",
                                           add_special_tokens=False)

            text_input = text_tensors_cat["input_ids"].squeeze(0)
            text_mask = text_tensors_cat["attention_mask"].squeeze(0)
            ...
            if self.max_seqlen is not None:
                ...
                text_mask.shape[0] <= max_seqlen else text_mask[:max_seqlen]
                labels = F.pad(labels, [max_seqlen - labels.shape[0], 0], mode='constant',
                               value=tokenizer.pad_token_id) if labels.shape[0] <= max_seqlen else labels[:max_seqlen]
